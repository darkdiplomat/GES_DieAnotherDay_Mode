...
class DieAnotherDay(GEScenario):
    ...
    def OnUnloadGamePlay(self):
        ...
        del self.timerTracker
        del self.tokenManager
        del self.radar
    ...
    def OnLoadGamePlay(self):
        ...
        self.CreateCVar(DieAnotherDay.CVAR_USED_RE_REVEAL_TIME, "10",
                        "How long the location of a used grave stone will be shown to both sides.")
    ...
    def OnCaptureAreaSpawned(self, area):
        ...
        self.radar.AddRadarContact(area, GEGlobal.RADAR_TYPE_OBJECTIVE, True, "sprites/hud/radar/RE",
                                   self.getSidesRadarColour(areasTeam, True))

        # Setup radar icon for idle RE:
        self.radar.SetupObjective(area, areasTeam, "", "", self.getSidesColour(areasTeam), 200, False)

        # If necessary, move the spawned RE:
        ID = area.GetGroupName()
        if self.REs.needsToBeMoved(ID):
            self.REs.moveToDeathLocation(ID)
    ...
    def OnCaptureAreaEntered(self, REArea, player, token):
        if REArea.GetTeamNumber() == player.GetTeamNumber() and self.REs.areaUsable(REArea): self.beginREInteraction(
            player, REArea, True)

    def OnCaptureAreaExited(self, REArea, player):
        if REArea.GetTeamNumber() == player.GetTeamNumber(): self.resurrections.playerHasExitedFriendlyREArea(REArea,
                                                                                                              player)

    def OnPlayerSpawn(self, player):
        self.pltracker.SetValue(player, self.trSpawned, True)
        GEUtil.ClientPrint(player, GEGlobal.HUD_PRINTTALK,
                           "^yIf you experience bugs in this Alpha version please report them on GitHub, "
                           "https://git.io/fpnmE")
    ...
    def OnRoundEnd(self):
        ...
        for timer in self.timerTracker.timers:
            timer.Stop()
        self.timerTracker.RemoveTimer()
    ...
    def OnPlayerKilled(self, victim, killer, weapon):
        ...
        killersTeam = None
        if killer:
            killersTeam = killer.GetTeamNumber()
    ...
    def OnPlayerEliminated(self, victim, killer=None, weapon=None):
        ...
        if killer is not None:
            ...
            if team == GEGlobal.TEAM_MI6:
                GEUtil.ClientPrint(None, GEGlobal.HUD_PRINTTALK, "#GES_GP_DAD_MI6_PLAYER_ELIMINATED",
                                   victim.GetPlayerName())
            else:
                GEUtil.ClientPrint(None, GEGlobal.HUD_PRINTTALK, "#GES_GP_DAD_JANUS_PLAYER_ELIMINATED",
                                   victim.GetPlayerName())
    ...
    def OnThink(self):
        ...
        if numMI6Players == 0 and numJanusPlayers == 0:
            ...
            self.teamWins(GEGlobal.TEAM_JANUS)
        elif numMI6Players > 0 and numJanusPlayers == 0:
            self.teamWins(GEGlobal.TEAM_MI6)
    ...
    def CanPlayerChangeTeam(self, player, oldTeam, newTeam, wasforced):
        if oldTeam != GEGlobal.TEAM_SPECTATOR and newTeam != GEGlobal.TEAM_SPECTATOR and not self.isEliminatedPlayer(
                player):
        ...
        if self.isEliminatedPlayer(player) and (newTeam == GEGlobal.TEAM_MI6 or newTeam == GEGlobal.TEAM_JANUS):
            teamChangeCheckTimer = DieAnotherDay.ExtCallbackTimer(self.timerTracker, self.observerTeamChangeCheck,
                                                                  player)
    ...
    def getEntHitByLRRLaser(user):
        ...
        return GEUtil.Trace(user.GetEyePosition(), traceEndVector,
                            GEUtil.TraceOpt.CAPAREA | GEUtil.TraceOpt.PLAYER | GEUtil.TraceOpt.WORLD | GEUtil.TraceOpt.WEAPON,
                            user)

    def IsInPlay(self, player):
        return player.GetTeamNumber() is not GEGlobal.TEAM_SPECTATOR and self.pltracker.GetValue(player,
                                                                                                 self.trSpawned) and not self.pltracker.GetValue(
            player, self.trEliminated)
    ...
    def getSidesRadarColour(self, side, isREIcon=True):
        if side == GEGlobal.TEAM_MI6:
            if isREIcon:
                return DieAnotherDay.mRERadarColour
            else:
                return DieAnotherDay.mUsedRERadarColour
        else:
            if isREIcon:
                return DieAnotherDay.jRERadarColour
            else:
                return DieAnotherDay.jUsedRERadarColour

    def getSidesResQueue(self, team):
        if team == GEGlobal.TEAM_MI6:
            return self.mResurrectionQueue
        else:
            return self.jResurrectionQueue
    ...
    def updateResQueuePlayerCount(self, team):
        ...
        for player in resQueue:
            GEUtil.UpdateHudProgressBar(player,
                                        ...
                                        title="#GES_GP_DAD_RESURRECTION_QUEUE_POSITION\r" + str(
                                            resQueue.index(player) + 1))
    ...
    def addPlayerToResurrectionQueue(self, player, team):
        teamsRQueue = None
        if team == GEGlobal.TEAM_MI6:
            teamsRQueue = self.mResurrectionQueue
        else:
            teamsRQueue = self.jResurrectionQueue
        # Player Insertion:
        if self.playerNotBot(player):
            positionOfBotNearestRQueueFront = self.getPositionOfBotNearestQueueFront(teamsRQueue)
            if positionOfBotNearestRQueueFront == -1:
                teamsRQueue.append(player)
            else:
    ...
    def removePlayerFromTeamsRQueue(self, player, team):
        rQueue = None
        if team == GEGlobal.TEAM_MI6:
            rQueue = self.mResurrectionQueue
        else:
            rQueue = self.jResurrectionQueue
    ...
    def beginREInteraction(self, player, REArea, proximityInteraction):
        ...
        if resurrection is None:
            self.resurrections.startNewResurrection(RE, player, proximityInteraction)
        else:
            if proximityInteraction:
                resurrection.proximityEnabled = True
            else:
                resurrection.LRREnabled = True
    ...
    def resurrectPlayerFromTeamIfTeamHasEliminatedPlayers(self, resurrector):
        ...
        if areasTeam == GEGlobal.TEAM_MI6 and len(self.mResurrectionQueue) != 0:
            resurrectedPlayer = self.mResurrectionQueue.pop(0)
        elif areasTeam == GEGlobal.TEAM_JANUS and len(self.jResurrectionQueue) != 0:
            resurrectedPlayer = self.jResurrectionQueue.pop(0)

        if resurrectedPlayer is not None:
    ...
    def decideWhereREWillBeLocated(self, victim, killer, weapon):
        ...
        if killer != None and weapon is not None:
    ...
    def performStaircaseGapCheck(self, player):
        def stepSearch(originVector):
            return GEUtil.Trace(originVector,
                                GEUtil.VectorMA(originVector, GEUtil.Vector(0, 0, -1),
                                                self.maxDistanceBetweenGroundAndJumpingPlayer),
    ...
    class HUDSurvivorCounts:
        ...
        def refresh(self, useOnThinkDelay=False):
            ...
            if self.displayed:
                if mPCountNeedsUpdate:
                    self.draw(GEGlobal.TEAM_MI6)
                elif mSCountNeedsUpdate:
                    self.updateDisplayedSurvivorCount(GEGlobal.TEAM_MI6)
                if jPCountNeedsUpdate:
                    self.draw(GEGlobal.TEAM_JANUS)
                elif jSCountNeedsUpdate:
                    self.updateDisplayedSurvivorCount(GEGlobal.TEAM_JANUS)

        def updateDisplayedSurvivorCount(self, team=None):
            if team is None or GEGlobal.TEAM_MI6:
                GEUtil.UpdateHudProgressBar(None, self.mSCountPBIndex, self.mSurvivorCount)
            if team is None or GEGlobal.TEAM_JANUS:
    ...
    class LRRTargetMonitor:
        ...
        def __init__(self, resurrectionP):
            ...
            self.targetCheckTimer = self.DAD.timerTracker.CreateTimer(
                "LRRTargetMonitor:" + self.resurrection.user.GetPlayerName())
        ...
        def drawLaser(self):
            GEUtil.CreateTempEnt(GEUtil.TempEnt.BEAM, origin=self.resurrection.user.GetEyePosition(),
                                 end=self.REContactVector, duration=self.drawnLaserDuration + 0.2,
                                 color=self.laserColour)
    ...
    class REDict:
        ...
        def spawnResurrectionEntity(self, areaID, team, afterSpawnMoveTo=None):
            ...
            if afterSpawnMoveTo is not None:
                self.REs[areaID].setMoveToLocation(afterSpawnMoveTo)
        ...
        def getTeamsRELocations(self, teamP):
            ...
            for RE in self.REs.values():
                if RE.team == teamP:
                    foundLocations.append(RE.getLocation())
        ...
        class RE:
            ...
            def delete(self):
                if self.pulseTimer is not None: self.pulseTimer.stop()
            ...
            def setupObjective(self, team):
                ...
                if team is None: team = area.GetTeamNumber()
            ...
            def setupYellowObjective(self):
                ...
                self.DAD.radar.SetupObjective(area, area.GetTeamNumber(), "", "", DieAnotherDay.RQPositionColour, 200,
                                              True)
            ...
            def needsToBeMoved(self):
                return self.needsToBeMovedTo is not None

            def move(self):
                if self.needsToBeMovedTo is None:
            ...
            def changeRadarIconAfterDelay(self, icon, colour, delay):
                timer = DieAnotherDay.ExtCallbackTimer(self.DAD.timerTracker, self.changRadarIconAfterDelayCB,
            ...
            class RingPulseTimer:
                ...
                def DrawNewRing(self):
                    GEUtil.CreateTempEnt(GEUtil.TempEnt.RING, origin=self.origin, framerate=15, duration=2, speed=10,
                                         width=0.33, amplitude=0.0, radius_start=0, radius_end=220,
                                         color=self.PULSE_COLOUR)
    ...
    class Resurrection:
        ...
        def __init__(self, DADP, REP, userP, startedAtCloseRange):
            ...
            if startedAtCloseRange:
                self.proximityEnabled = True
            else:
                self.LRREnabled = True

        def getID(self):
            return self.RE.ID + ":" + self.user.GetPlayerName()

        def start(self):
            if self.timer.state == Timer.STATE_STOP:
                self.timer.Start(self.DAD.resurrectionTime, False)
            else:
                GEUtil.DevWarning(
                    "Start() has been called for a resurrection which is already running, so it was ignored:" + self.getID() + "\n")
            return False
        ...
        def resurrectionFailed(self):
            ...
            if self.hasUserDisconnected is False:
                ...
                if usersResCount == 0:
                    ...
                    GEUtil.UpdateHudProgressBar(self.user, self.DAD.resurrectionPBIndex, 0, str(usersResCount),
                                                self.DAD.getSidesColour(self.team))
            ...
            if update_type != Timer.UPDATE_STOP and not GEMPGameRules.IsIntermission():
                if self.RE.disabled or self.RE.used:
                    ...
                    if update_type == Timer.UPDATE_START:
                        ...
                        usersResCount = self.DAD.resurrections.getPlayersResurrectionCount(self.user)
                        if usersResCount == 1:
                            GEUtil.InitHudProgressBar(self.user, self.DAD.resurrectionPBIndex, "1",
                                                      GEGlobal.HUDPB_SHOWBAR, self.DAD.resurrectionTime, -1,
                                                      DieAnotherDay.resBarY, 120, 16,
                                                      self.DAD.getSidesColour(self.team))
                        else:
                            GEUtil.UpdateHudProgressBar(self.user, self.DAD.resurrectionPBIndex, 0, str(usersResCount),
                                                        self.DAD.getSidesColour(self.team))
                        ...
                        if self.hasUserDisconnected or self.hasUserDied or self.team != self.user.GetTeamNumber():
                            self.resurrectionFailed()
                        ...
                        # If it hasn't failed:
                            ...
                            if self.LRREnabled and self.LRRMonitorExists is False:
                                # Start a LRR target monitor:
                                LRRTargetMonitor = DieAnotherDay.LRRTargetMonitor(self)
                                LRRTargetMonitor.start()
                                self.LRRMonitorExists = True
                            elif self.LRREnabled is False and self.LRRMonitorExists:
                                # Stop LRR Target Monitor
                                self.DAD.playersLRRTargetMonitor[self.user].delete()
                                self.LRRMonitorExists = False
                            ...
                            if self.DAD.resurrections.isPlayersMostRecentResurrection(self.user, self):
                                ...
                                GEUtil.UpdateHudProgressBar(self.user, DieAnotherDay.resurrectionPBIndex,
                                                            int(self.timer.GetCurrentTime()), str(
                                        self.DAD.resurrections.getPlayersResurrectionCount(self.user)))
                    # End a succesful resurrection:
                        ...
                        if self.DAD.resurrections.getPlayersResurrectionCount(self.user) == 1:
                            ...
                            GEUtil.UpdateHudProgressBar(self.user, self.DAD.resurrectionPBIndex,
                                                        self.DAD.resurrectionTime, "0", DieAnotherDay.RQPositionColour)
                            # 15.Remove the resurrection progress bar.
                            progressBarRemovalTimer = DieAnotherDay.ExtCallbackTimer(self.DAD.timerTracker,
                                                                                     self.DAD.delayedResurrectionPBRemovalIfNoActiveResurrectionsAfterDelay,
                                                                                     self.user)
                        ...
                        if (resurrectedPlayer is not None):
                            ...
                            GEUtil.EmitGameplayEvent("DAD_Resurrection", "%s" % playersName, "%i" % self.team,
                                                     "%s" % self.user.GetPlayerName())
                            if self.team == GEGlobal.TEAM_MI6:
                                GEUtil.ClientPrint(None, GEGlobal.HUD_PRINTTALK, "#GES_GP_DAD_MI6_PLAYER_RESURRECTED",
                                                   playersName)
                            else:
                                GEUtil.ClientPrint(None, GEGlobal.HUD_PRINTTALK, "#GES_GP_DAD_JANUS_PLAYER_RESURRECTED",
                                                   playersName)
                            # 20.After a few seconds of being yellow, change the "used RE" icons colour to be the used RE's side's colour.
                            self.RE.changeRadarIconAfterDelay("sprites/hud/radar/run",
                                                              self.DAD.getSidesRadarColour(self.team, False), 3.0)
                            ...
                            self.DAD.REs.deleteRE(self.RE.ID)
